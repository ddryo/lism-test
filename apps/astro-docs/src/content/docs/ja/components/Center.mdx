---
title: 'Center'
description: ...
---

import { Center, Frame, Alert, Box, Text } from '@loos/lism-core';
import { Reference, MemoBadge, HelpText, DammyText } from "~/components";
import { Preview, PreviewArea, PreviewTitle, PreviewCode } from "~/components/Preview";


**要素**を上下左右中央揃えで配置するためのモジュールです。


## 主なProps

{/* | プロパティ | 説明 | 有効な値 |
|---|---|---|
| `size`| 指定した値によって自動でpropが適用されます。 |`cover` or `content`| */}

[`<Grid>`で使えるprops](/ja/components/grid#props)が使えます。

{/* - `gap` : 子要素間の余白を指定します。 */}


## Import

<Preview>
	<PreviewCode slot='tab' label='React'>
```jsx
import { Center } from '@loos/lism-core';
```
	</PreviewCode>
	<PreviewCode slot='tab' label='Astro'>
```jsx
import { Center } from '@loos/lism-astro';
```
	</PreviewCode>
</Preview>


## Usage

Centerが**高さを持たない**場合は、コンテンツを水平方向に中央揃えで配置するだけとなります。
{/* <Alert preset="info">
水平方向の中央揃えだけでいい場合は、`<Stack ai="center">`でも可能です。
</Alert> */}

{/* 水平方向に関しては、いわゆる『内在的な』中央揃え（テキスト幅が小さければ左右中央揃え、長ければ普通に左寄せ状態と同じ状態）になります。 */}

<Preview>
	<PreviewTitle>
		高さの設定が特にない時
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Center p='box'>
			<Text fz='l'>TEXT</Text>
		</Center>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx
		<Center p='box'>
			<Text fz='l'>TEXT</Text>
		</Center>
		```
	</PreviewCode>
	<PreviewCode slot='tab' label='HTML'>
		```jsx
		<div class="l--center -p:box">
			<p class="-fz:l">TEXT</p>
		</div>
		```
	</PreviewCode>
</Preview>

<Preview>
	<PreviewTitle>
		長いテキストの改行は左寄せになるので、テキストの読みやすさを損ないません。
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Center p='box' gap={20}>
			<Text fz='l'>TEXT</Text>
			<DammyText length='xs' lang='ja'/>
			<DammyText length='m' lang='ja'/>
		</Center>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx
		<Center p='box' gap={20}>
			<Text fz='l'>TEXT</Text>
			<p>...</p>
			<p>...</p>
		</Center>
		```
	</PreviewCode>
	<PreviewCode slot='tab' label='HTML'>
		```jsx
		<div class="l--center -p:box -gap:20">
			<p class="-fz:l">TEXT</p>
			<p>...</p>
			<p>...</p>
		</div>
		```
	</PreviewCode>
</Preview>



### コンテンツを水平・垂直方向の中央に配置する

Centerに**高さがある時**、垂直方向に対しても中央揃えになります。

<Preview>
	<PreviewTitle>
		高さを設定した時
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Center p={30} gap={20} h='50vh'>
			<Text fz='l'>TEXT</Text>
			<DammyText length='xs' lang='ja'/>
			<DammyText length='m' lang='ja'/>
		</Center>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx "h='50vh'"
		<Center h='50vh' p={30} gap={20}>
			<Text fz='l'>TEXT</Text>
			<p>...</p>
			<p>...</p>
		</Center>
		```
	</PreviewCode>
	<PreviewCode slot='tab' label='HTML'>
		```jsx
		<div class="l--center -p:30 -h: -gap:20" style="--h:50vh">
			<p class=" -fz:l">TEXT</p>
			<p>ロレムイプサムの座り雨。</p>
			<p>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。ブラインド行うにはいくつかの重要な事柄が流れます。長年にわたり、私は学区と長寿であれば。</p>
		</div>
		```
	</PreviewCode>
</Preview>

水平方向の中央揃えを解除して、垂直方向のみ中央揃えにしたい場合は、`<Center>`に対して`justify-items:start`を指定してください。

<Preview>
	<PreviewTitle>
		垂直方向のみ中央揃えに
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Center p={30} gap={20} h='50vh' ji='start'>
			<Text fz='l' fw='700'>TEXT</Text>
			<DammyText length='xs' lang='ja'/>
			<DammyText length='m' lang='ja'/>
		</Center>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx "ji='start'" "h='50vh'"
		<Center ji='start' h='50vh' p={30} gap={20}>
			<Text fz='l' fw='700'>TEXT</Text>
			<p>...</p>
			<p>...</p>
		</Center>
		```
	</PreviewCode>
	<PreviewCode slot='tab' label='HTML'>
		```jsx "-ji:s"
		<div class="l--center -p:30 -h: -gap:20 -ji:s" style="--h:50vh">
			<p class=" -fz:l">TEXT</p>
			<p>ロレムイプサムの座り雨。</p>
			<p>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。ブラインド行うにはいくつかの重要な事柄が流れます。長年にわたり、私は学区と長寿であれば。</p>
		</div>
		```
	</PreviewCode>
</Preview>


{/* ## `size='cover'`

`<Center size='cover'>`は`<Center minW='100%' minH='100%'>`のエイリアスです。

`<Center>`が親要素のサイズに合わせて広がるようになります。

<Preview>
	<PreviewTitle>
		`size='cover'`
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Frame aspect='16/9'>
			<Center size='cover' bgc='pale'>
				<Text fz='l' fw='700'>`size='cover'`</Text>
				<Text fz='xs'>= `.-minW100`,`.-minH:100`</Text>
			</Center>
		</Frame>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx "size='cover'"
		<Frame aspect='16/9'>
			<Center size='cover' bgc='pale'>
				<p>...</p>
			</Center>
		</Frame>
		```
	</PreviewCode>
	<PreviewCode slot='tab' label='HTML'>
		```jsx "-minW:100 -minH:100"
		<div class="l--frame -aspect:16/9">
			<div class="l--center -minW:100 -minH:100 -bgc:pale">
				<p>...</p>
			</div>
		</div>
		```
	</PreviewCode>
</Preview>


<Alert preset="info">
`width`, `height` ではなく `min-width`, `min-height` を指定しているのは、`<Center>`のコンテンツが親要素よりも大きくなった時にスクロールに問題が発生するためです。（詳しくは後述します）
</Alert>



### size='content'


`<Center size='content'>`は`<Center w='fit-content' h='fit-content' mx='auto'>`のエイリアスです。

`<Center>`のサイズは内部コンテンツに依存するようになり、`<Center>`自身が親要素に対して水平方向(inline方向)に中央揃えで配置されるようになります。


<Preview>
	<PreviewTitle>
		`size='content'`
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Center size='content' p='box' bgc='pale' shadow='1'>
			<Text ff='emoji'>💡</Text>
			<Text fw='bold'>`size='content'`</Text>
		</Center>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx "size='content'"
		<Center size='content' p='box' bgc='pale' shadow='1'>
			<Text ff='emoji'>💡</Text>
			<p>...</p>
		</Center>
		```
	</PreviewCode>
	<PreviewCode slot='tab' label='HTML'>
		```jsx "-w:fit -h:fit -mx:a"
		<div class="l--center -w:fit -h:fit -mx:a -p:box -bgc:pale -bxsh:1">
			<p class="-ff:emoji">💡</p>
			<p>...</p>
		</div>
		```
	</PreviewCode>
</Preview> */}


## 内部を スクロール可 にしたい時の注意点

"画面サイズに余裕がある時は上下中央揃えにしたいが、内部のコンテンツが長くて画面サイズが小さい時にはみ出すのでスクロールできるようにしておく必要がある。"

という場合もあると思いますが、この時は少し注意が必要です。

<Alert preset='alert'>
`grid`や`flex`に`align-items:center`を適用させて上下中央揃えを実装する方法では、スクロール時に上端が見えなくなるという問題があります。
</Alert>

Lismの`<Center>` も `grid` を使ってるので、同じ問題が発生します。

以下のDEMOエリアをリサイズして挙動を確認してみてください。

<Preview>
	<PreviewTitle>
		スクロール時にコンテンツが全て見えない 😱
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Center px={30} py={50} gap={20} h='45vh' ovy='auto' bd>
			<Text fz='l' fw='700'>DEMO</Text>
			<DammyText length='l' />
			<DammyText length='xl' />
		</Center>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx "\<Center" "h='45vh' ovy='auto'"
		<Center px={30} py={50} gap={20} h='45vh' ovy='auto' bd>
			<Text fz='l' fw='700'>DEMO</Text>
			<DammyText length='l' />
			<DammyText length='xl' />
		</Center>
		```
	</PreviewCode>
</Preview>

これを回避するには、親要素側に具体的な**高さ**と`overflow-y:auto`を指定し、`<Center>`は`min-height:100%`を指定する、というアプローチが必要です。

<Alert preset='alert'>
`height:100%` だとうまくいかないので注意
</Alert>

<Preview>
	<PreviewTitle>
		スクロール時、正常にコンテンツが見える 😄
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Box px={30} py={50} h='45vh' ovy='auto' bd>
			<Center gap={20} minH='100%'>
				<Text fz='l' fw='700'>DEMO</Text>
				<DammyText length='l' />
				<DammyText length='xl' />
			</Center>
		</Box>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx "\<Center" "h='45vh' ovy='auto'" "minH='100%'"
		<Box px={30} py={50} h='45vh' ovy='auto' bd>
			<Center gap={20} minH='100%'>
				<Text fz='l' fw='700'>DEMO</Text>
				<DammyText length='l' />
				<DammyText length='xl' />
			</Center>
		</Box>
		```
	</PreviewCode>
</Preview>



{/* ## Centerで使用されているCSSについて

垂直中央揃えの指定方法については、次のように、直下の`:first-`,`:last-`要素の`margin`に対して`auto`を指定することで実装しています。

```scss
.l--center > :first-child {
	margin-top: auto;
}
.l--center > :last-child {
	margin-bottom: auto;
}
```

<Alert preset="info">
	`justify-content:center` にするだけでも垂直方向にセンター寄せできますが、それだと**スクロール時に正常な動作にならない**ため、`margin`で実装しています。
</Alert>


次のエリアをリサイズしてスクロール時の挙動の違いを比較してみてください。


<Preview>
	<PreviewTitle>
		垂直中央揃えの実装方法の比較
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Columns cols="2" fz="s" lh="s">
			<Frame aspect="4/3" bgc="pale">
				<Center h="100%" p='box'>
					<Text fw="700">margin</Text>
					<DammyText length='l' mbs={20}/>
				</Center>
			</Frame>
			<Frame aspect="4/3" bgc="pale">
				<Stack ai="center" jc="center" h="100%" p='box'>
					<Text fw="700">justify-content</Text>
					<DammyText length='l' mbs={20}/>
				</Stack>
			</Frame>
		</Columns>
	</PreviewArea>
		<PreviewCode slot='tab' label='React, Astro'>
		```jsx {3, 9}
		<Columns cols="2" fz="s" lh="s">
			<Frame aspect="4/3" bgc="pale">
				<Center h="100%" p='box'>
					<Text fw="700">margin</Text>
					...
				</Center>
			</Frame>
			<Frame aspect="4/3" bgc="pale">
				<Stack ai="center" jc="center" h="100%" p='box'>
					<Text fw="700">justify-content</Text>
					...
				</Stack>
			</Frame>
		</Columns>
		```
	</PreviewCode>
</Preview>


このように、`margin`で実装している方はスクロールして正常にコンテンツが全て見えるようになっていますが、`justify-content`での実装では、コンテンツが見えない状態になっていることがわかります。


ただし、**実はまだ注意が必要**です。よく見ると下側のpadding部分まではスクロールできていません。

これを改善するには、親の`<Frame>`でスクロールを発生させるのではなく、`<Center>`自体をスクロールする必要があります。

そのためには、`overflow-y`を`auto`(または`scroll`)に指定しておく必要があります。

以下のDEMOエリアをリサイズして挙動を比較してみてください。

<Preview>
	<PreviewTitle>
		`overflow-y:auto`の有無の差
	</PreviewTitle>
	<PreviewArea resize p={10}>
		<Columns cols="2" fz="s" lh="s">
			<Frame aspect="4/3" bgc="pale">
				<Center h="100%" px={30} py={40} ovy="auto">
					<Text fz='l' fw="700">ovy "auto"</Text>
					<DammyText length='l' mbs={20} />
				</Center>
			</Frame>
			<Frame aspect="4/3" bgc="pale">
				<Center h="100%" px={30} py={40}>
					<Text fz='l' fw="700">ovy なし</Text>
					<DammyText length='l' mbs={20} />
				</Center>
			</Frame>
		</Columns>
	</PreviewArea>
	<PreviewCode slot='tab' label='React, Astro'>
		```jsx {3,9}
		<Columns cols="2" fz="s" lh="s">
			<Frame aspect="4/3" bgc="pale">
				<Center h="100%" px={30} py={40} ovy="auto">
					<Text fz='l' fw="700">ovy "auto"</Text>
					...
				</Center>
			</Frame>
			<Frame aspect="4/3" bgc="pale">
				<Center h="100%" px={30} py={40}>
					<Text fz='l' fw="700">ovy なし</Text>
					...
				</Center>
			</Frame>
		</Columns>
		```
	</PreviewCode>
</Preview>


このため、`size="cover"`にはデフォルトで`overflow-y: auto;`が指定されています。 */}
