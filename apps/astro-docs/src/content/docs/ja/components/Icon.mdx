...
<Preview>
	<PreviewTitle>コンポーネント用のpropsを`asProps`に指定する</PreviewTitle>
	<PreviewArea resize p={20}>
...
<Preview>
	<PreviewTitle>`svg`を`children`に渡す</PreviewTitle>
	<PreviewArea resize p={20}>
...
<Preview>
	...
	<PreviewArea resize p={20}>
		...
		<PreviewCodes>
			<PreviewCode label="React">
				```jsx
				<Icon emoji="✍️" fz="2rem" />
				<Icon emoji="💡" fz="2rem" />
				```
			</PreviewCode>
			<PreviewCode label="HTML">
				```html
...
<Preview>
	<PreviewTitle>`isInline` の使用</PreviewTitle>
	<PreviewArea resize p={20}>
