...
import { Alert, Box, Note, Text, Layouter, Stack, Flex, Container, Frame, Button, Center } from '@loos/lism-core';
...
<Alert preset='alert'>
Design Stateに対応するコンポーネント用のPropsは、`<Layouter>`系コンポーネントでのみ有効です。(`<Core>`ではサポートされていません)
</Alert>
...
<Preview>
	<PreviewTitle>例</PreviewTitle>
	<PreviewArea p={20}>
		<Layouter isContainer='s' bd='guide'>
