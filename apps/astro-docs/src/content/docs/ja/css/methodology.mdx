...
<table className="table--solid">
	...
	<tbody>
		...
		<tr>
			<td rowSpan={2} style={{verticalAlign: 'top'}}>Objects</td>
			<td>Element</td>
			<td rowSpan={2}>具体的な役割を持つコンポーネントなど</td>
			<td rowSpan={2}>0-1-0</td>
		</tr>
		{/* <tr>
			<td>Block</td>
...
基本的に、最大詳細度を超えないようにセレクタを調整してこのレイヤー順番で読み込みます。  
それぞれカスケードレイヤー（`@layer`）を定義して読み込むようにすることで詳細度をあまり気にせずに書けるようになります。
