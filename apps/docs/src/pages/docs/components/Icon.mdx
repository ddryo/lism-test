import { Container, Badge, Cluster, Icon, Center, FluidFix } from '@loos/lism-core';
import {
	Smiley,
	Horse,
	ChatCenteredDots,
...
- `scale` アイコンの scaleを指定できます。

### 補足説明

- `icon`に文字列を渡した場合、以下の2パターンの挙動に分かれます。
	1. プリセット定義済みの文字列 → 準備済みのアイコンの呼び出しを行います。（数は少ないですが、追加のパッケージをインストールせずにアイコンを呼び出せます。）
		- ※ プリセットアイコンには[Phosphor Icons](https://phosphoricons.com/)を使用しています。
	2. その他の文字列 → `data-icon`にその文字列が出力され、CSSと併用してアイコンの表示を行うことができます。  
...
事前`@loos/lism-core`に組み込み済みのアイコンを呼び出します。

- `"alert"`: <Icon d="inline" size="1.25em" icon="alert" />
- `"warning"`: <Icon d="inline" size="1.25em" icon="warning" />
- `"info"`: <Icon d="inline" size="1.25em" icon="info" />
- `"help"`: <Icon d="inline" size="1.25em" icon="help" />
- `"check"`: <Icon d="inline" size="1.25em" icon="check" />
- `"lightbulb"`: <Icon d="inline" size="1.25em" icon="lightbulb" />
- `"note"`: <Icon d="inline" size="1.25em" icon="note" />
- `"good"`: <Icon d="inline" size="1.25em" icon="good" />
- `"bad"`: <Icon d="inline" size="1.25em" icon="bad" />

<Demo>
	<Demo.Title>プリセット指定例</Demo.Title>
	<Demo.Preview resize p={20}>
		<Cluster gap={20}>
			<Icon icon="alert" size="1.25em" />
			<Icon icon="warning" size="1.25em" />
			<Icon icon="help" size="1.25em" />
			<Icon icon="check" size="1.25em" />
			<Icon icon="lightbulb" size="1.25em" />
		</Cluster>
	</Demo.Preview>
	<Demo.Code>
```jsx
...
Lismコアでは`"accordion-toggle"`のみ用意しています。

<Demo>
	<Demo.Title>`icon="accordion-toggle"`</Demo.Title>
	<Demo.Preview resize p={20}>
		<Icon icon="accordion-toggle" size="2em" />
	</Demo.Preview>
	<Demo.Code>
```jsx
<Icon icon="accordion-toggle" size="2em" />
...
<Demo>
	...
	<Demo.Preview resize p={20}>
		<Flex ai="center" gap={20} bd={{ c:"red" }} p='box:s' radius="4">
			<Icon icon={WarningCircle} size="10%" c="red" flexItem={{ shrink:'0' }} />
			<DammyText length="m" />
		</Flex>
	</Demo.Preview>
	<Demo.Code>
