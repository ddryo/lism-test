...
## 要検討
- アコーディオンと同様に、isFlow, flowGap をデフォルトセットする？
...
```js
import { Tab, TabItem } from '@lism/core';
```

## DEMO

<Demo>
	<Demo.Title>`<Tabs>`の例</Demo.Title>
 ...
 <Demo>
	<Demo.Title>最初に開くタブを指定、高さが変わらないようにする"</Demo.Title>
	<Demo.Preview resize p={20}>
		<Tabs keepHeight defaultIndex={1}>
			<TabItem label="title1" isFlow flowGap='s'>
				<p>index:0</p>
				<DammyText />
			</TabItem>
			<TabItem label="title2" isFlow flowGap='s'>
				<p>index:1</p>
				<DammyText />
				<DammyText length="l" />
			</TabItem>
