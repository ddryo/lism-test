import { Box, Flex, Cluster, Stack, Item } from '@lism/core';
...
<Demo>
	...
	<Demo.Preview resize p={20}>
		<Box d="flex" gap={20} className='has:itemGuide'>
			<p>A</p>
			<p>B</p>
			<p>C</p>
		</Box>
...
{/* - flex.isInline: `display:inline-flex` になります。 */}
{/* 
<Note caption="Memo" preset="memo">
	alignItem,justifyContentなどもここに含めるべきか要検討
</Note> */}

<Demo>
	<Demo.Title>`flex`の指定例</Demo.Title>
	<Demo.Preview resize p={20}>
		<Box d='flex' flex={{ wrap:'wrap', direction:'row-reverse', jc:"center" }} gap={20} className='has:itemGuide'>
			<p>A</p>
			<p>B</p>
			<p>C</p>
		</Box>
...
<Demo>
	...
	<Demo.Preview resize p={20}>
		<Flex wrap='wrap' direction='row-reverse' jc="center" gap={20} className='has:itemGuide'>
			<p>A</p>
			<p>B</p>
			<p>C</p>
		</Flex>
...
<Demo>
	...
	<Demo.Preview resize p={20}>
		<Cluster direction='row-reverse' jc="center" gap={20} className='has:itemGuide'>
			<p>A</p>
			<p>B</p>
			<p>C</p>
		</Cluster>
...
- `fxb`: flex-basisの値を指定できます。
- `fxg`: flex-growの値を指定できます。
- `fxsh`: flex-shrinkの値を指定できます。
- `fx`: flexの値を指定できます。
...
<Demo>
	...
	<Demo.Preview resize p={20}>
		<Flex jc="center" gap={20} className='has:itemGuide'>
...
<Demo>
	...
	<Demo.Preview resize p={20}>
		<Flex fxw="nowrap" gap={20} className='has:itemGuide'>
			<Item fx="1 1 50%" bgc="pale">
				<DammyText length="xs"/>
			</Item>

			<Item fxsh={["1", "0"]} fxb={["50%", "60%"]} bgc="pale">
				<DammyText length="m"/>
			</Item>
		</Flex>
	</Demo.Preview>
